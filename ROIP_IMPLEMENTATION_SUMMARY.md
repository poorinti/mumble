...
## Operations, Audit และ Real-time

- ทุกคำสั่งปฏิบัติการสำคัญจะบันทึกลง PostgreSQL `audit_events` อัตโนมัติพร้อมผู้สั่ง, เวลา, IP, สถานี และเป้าหมาย โดยไม่เก็บรหัสผ่านใน Audit เช่น เตะ, แบน, ยกเลิกแบน, เปลี่ยนรหัสเซิร์ฟเวอร์/ห้อง, สร้าง/ลบ/เปลี่ยนชื่อห้อง, ย้ายผู้ใช้, รีสตาร์ต และสร้าง/ลบบัญชี Mumble
- Redis ใช้เป็นทั้ง cache สถานะและคิวงานเบื้องหลัง (`roip:operations:queue`) งานตรวจสถานีและล้างเสียงจึงไม่ขวางหน้าจอหลัก
- WebSocket ส่งสถานะผู้ใช้/ห้องทุกช่วงตรวจ และส่งเหตุการณ์เมื่อคำสั่งสำเร็จ เพื่อให้หน้าต่างที่เปิดอยู่รีเฟรชข้อมูลสดโดยไม่รอรอบ polling ปกติ
- ระบบตรวจ Mumble ทุก `STATUS_CHECK_SECONDS` วินาที (ค่าเริ่มต้น 15) และบันทึก/แจ้งสถานะ ONLINE หรือ OFFLINE เมื่อสถานะเปลี่ยน
- ระบบลบไฟล์เสียงเฉพาะนามสกุลเสียงที่เก่ากว่า `AUDIO_RETENTION_DAYS` (ค่าเริ่มต้น 30 วัน) ทุก `AUDIO_RETENTION_CHECK_MINUTES` นาที พร้อมอัปเดต `audio_assets.deleted_at` และ Audit
- Admin เปิดปุ่ม **🩺 Health** เพื่อดู CPU, RAM, Disk, PostgreSQL, Redis, พื้นที่ไฟล์เสียง และ Online/Offline ของทุกสถานี รวมถึงสั่งตรวจ retention ได้ทันที
- ในหน้าต่างเดียวกันมี Audit Log แบบแบ่งหน้า, รายงานรายวันจำนวนข้อความ/เสียง/ไฟล์เสียง/เหตุการณ์ และปุ่ม Export CSV ตามช่วงวันและสถานี
- Command Log หน้า Dashboard เปลี่ยนเป็น cursor pagination และ virtual rendering: โหลดครั้งละ 100 รายการและสร้าง DOM เฉพาะแถวที่มองเห็น จึงเปิด Log จำนวนมากได้โดยไม่หน่วง

### ค่าที่ต้องกำหนดใน `.env`

```env
REDIS_PASSWORD=รหัสผ่าน_redis_ที่สุ่มยาว
STATUS_CHECK_SECONDS=15
AUDIO_RETENTION_DAYS=30
AUDIO_RETENTION_CHECK_MINUTES=60
AUDIO_STORAGE_WARN_PERCENT=80
```

หลังแก้ `.env` ให้สั่ง build และ start ใหม่ด้วยคำสั่ง Docker Compose ที่เครื่องรองรับ (`docker compose ...` หรือ `docker-compose ...`) ระบบจะสร้าง service `roip-redis` และ volume `redis-data` ให้อัตโนมัติ
...
- `quiet_modes` — ห้องที่เปิดโหมดเงียบและชื่อผู้บัญชาการที่ได้รับอนุญาตให้พูด
...
## Quiet Mode, เวลาอยู่ห้อง และ Mobile Swipe (20 สิงหาคม 2026)

- เพิ่มปุ่ม `Q` ที่หัวห้องเพื่อเปิด Quiet Mode เลือกผู้บัญชาการ และยกเลิกโหมดจาก Popup เดียว
- เมื่อเปิด Quiet Mode ระบบปิดไมค์ทุกคนในห้อง ยกเว้นผู้บัญชาการ และตรวจผู้เข้าใหม่ซ้ำทุก 15 วินาที
- การตั้งค่า Quiet Mode เก็บใน PostgreSQL จึงยังทำงานต่อหลังรีสตาร์ต Command Center
- รายชื่อผู้ใช้แสดงเวลาที่อยู่ในห้องปัจจุบัน; เมื่อย้ายห้องหรือเชื่อมต่อใหม่จะเริ่มนับรอบใหม่
- การเปิด/ปิด Quiet Mode ถูกบันทึกทั้ง Command Log และ Audit Log
- มือถือแสดงข้อมูลทีละมุมมองเต็มพื้นที่: `แผนผัง`, `หน้าแชท`, `หน้า Log`, `ประวัติ`
- ปัดซ้าย–ขวาบนพื้นที่เนื้อหา หรือกดลูกศรบนแถบเปลี่ยนหน้าเพื่อสลับมุมมอง โดยไม่ออกจาก SPA
- Header มือถือเลื่อนแนวนอนได้ จึงไม่ตัดปุ่มควบคุม ส่วนปุ่มคำสั่งผู้ใช้จัดเป็นหลายบรรทัดและเพิ่มพื้นที่สัมผัส
- คู่มือ Help อธิบายปุ่ม Quiet Mode, ตัวนับเวลา และวิธีปัดหน้าบนมือถือแล้ว
- หน้า `ประวัติ` บนมือถือให้หน้า Search ภายใน iframe เลื่อนแนวตั้งได้ จึงเข้าถึงตัวกรอง ช่องคำค้น และปุ่มค้นหาได้ครบ
- Navbar มือถือย่อเป็น 2 แถว: สถานะแบบสั้น และชื่อสถานีพร้อมปุ่มไอคอนทุกคำสั่ง จึงไม่ต้องเลื่อนแนวนอนเพื่อหาเมนู
- ทุกมุมมองบนมือถือเลื่อนขึ้น–ลงได้ตามปกติ ขณะที่การปัดซ้าย–ขวายังคงใช้สลับมุมมอง Dashboard; Log, AI และ History กำหนดความสูงที่เลื่อนได้ชัดเจน
- แถบเปลี่ยนมุมมองบนมือถือมีปุ่ม “ก่อนหน้า/ถัดไป” ขนาดใหญ่ กดได้ง่าย และรองรับการปัดนิ้วซ้าย–ขวาด้วยระยะปัดที่สั้นลง โดยยังแยกจากการเลื่อนขึ้น–ลงปกติ
